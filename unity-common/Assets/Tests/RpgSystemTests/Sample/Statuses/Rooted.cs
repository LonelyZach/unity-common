...
{
  internal class Rooted : Status, IEcsObserver<SampleState>
  {
    public override string Name => nameof(Rooted);

    public IEnumerable<IEcsObserver<SampleState>.Register> Observers()
    {
      yield return IEcsObserver<SampleState>.On<TranslationModule.TranslationStarted>((state, evt) =>
      {
        state.Characters
